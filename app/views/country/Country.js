...
import { getIsStateLoading, getStateError, getStateData } from "../../redux/state/selector";
      ...
      <div className="ctry10boxesContainer">
        {
          Object.keys(cardData).map((key, index) => {
            return <div key={`${key + index}`}><Card title={key} value={cardData[ key ] || 'Unknown'} /></div>
          })
        }
      </div>
    );
...
export default connect(mapStateToProps, { fetchStates })(Country);
